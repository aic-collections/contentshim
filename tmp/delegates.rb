...
class CustomDelegate
  ...
  def authorize(options = {})
    ...
    if request_uri.end_with?("/info.json")
        return true
    end
    
    uri_parts = request_uri.split('/')
    uri_firsthalf = uri_parts[0..6].join('/')
    uri_firsthalf = uri_firsthalf.gsub('http:', 'https:')
    # This will rewrite production urls but not staging or testing urls.
    uri_firsthalf = uri_firsthalf.gsub('lakeimagesweb.artic.edu', 'www.artic.edu')
    uri_secondhalf = uri_parts[8..9].join('/')
    
    size = uri_parts[7]
    mustcheck = false
    if size.include? 'full'
        mustcheck = true
    elsif size.include? 'max'
        mustcheck = true
    elsif size.include? ',' and !size.include? ':'
        mustcheck = true
    end
    
    if identifier.include? "zzzzzzz"
        mustcheck = false
    end
    ...
    if mustcheck
        newsize = size
        if newsize.include? ','
            newsize = newsize.split(',')[0]
        end
        if newsize.include? '!'
            newsize = newsize.sub! '!', ''
        end
        if newsize.include? 'full' or newsize.include? 'max'
            newsize = "3000"
        end
        if newsize != "" and newsize.to_i > 843
            uri = 'http://localhost:8000/assets/' + CGI.escape(identifier) + '/public_domain'
            uri = URI.parse(uri)
            ...
            if response.code.to_i == 200
                ...
                if rjson["is_public_domain"] == false
                    uri_size = "843,"
                    new_uri = uri_firsthalf + "/" + uri_size + "/" + uri_secondhalf
                    {"status_code" => 307, "location" => new_uri}
                else
                    true
                end
            else
        ...
        # This may only permit the pattern with ':' in it.  Must understand
        # these types of IIIF requests better.
        true
    end
  end
